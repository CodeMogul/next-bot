...
void Motion::rotate_to(float angle)
{
    posRefL = angle*R*(wheel_l->ENC_COUNT/(2*3.1416*wheel_l->r));
    posRefR = -angle*R*(wheel_r->ENC_COUNT/(2*3.1416*wheel_r->r));
    mode = ROTATE_TO;
    flush_all();
}

void Motion::wheel_omega(float omega_l, float omega_r)
{
  omegaRefL = omega_l;
  omegaRefR = omega_r;
  mode = WHEEL_OMEGA;
